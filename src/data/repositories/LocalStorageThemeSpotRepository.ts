...
const STORAGE_KEY = "theme_spots_likes";

const SEED_DATA: ThemeSpot[] = [
    {
        id: "animation-1",
        themeId: "animation",
        title: "너의 이름은",
...
export class LocalStorageThemeSpotRepository implements IThemeSpotRepository {
    ...
    private enrichWithLikes(spots: ThemeSpot[]): ThemeSpot[] {
    ...
    async search(query: string, themeId?: ThemeCategoryId, _userId?: string): Promise<ThemeSpot[]> {
        ...
        if (themeId) {
            filtered = filtered.filter(s => s.themeId === themeId);
        }
